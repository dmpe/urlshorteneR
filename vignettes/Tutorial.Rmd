...
This package is wrapping [Bit.ly](https://www.bitly.com) API & [Goo.gl](https://goo.gl). In order to begin shortening URLs and downloading usefull statistics, there is one requirement, which is to authenticate using OAUTH2 for each service. 
...
bitly_token <- readRDS(btoken_path)
googl_token <- readRDS(gtoken_path)
