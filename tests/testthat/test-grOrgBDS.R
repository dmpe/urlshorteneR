...
test_that("You cannot retrieve specific organization because there is empty string", {
  expect_error(bitly_retrieve_org(), "organization_id must not be empty string, NA or NULL")
})
...
test_that("You cannot retrieve shorten counts for organization because there is empty string", {
  expect_error(bitly_retrieve_org_shorten_counts(), "organization_id must not be empty string, NA or NULL")
})

test_that("Retrieve shorten counts for organization by group", {
  organization_guid <- bitly_retrieve_orgs()
  by_grp <- bitly_retrieve_org_shorten_counts_by_group(organization_guid$guid)
  expect_gt(by_grp$metrics$value, 40)
})
...
test_that("we can get group's usage limits", {
  ui <- bitly_user_info()
  group_limits <- bitly_retrieve_group_feature_usage(group_id = ui$default_group_guid[1])
