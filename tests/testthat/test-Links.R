...
test_that("Query for a Bitlink", {
  ll <- bitly_retrieve_bitlink(access_token = Sys.getenv("access_token"), bitlink = "1.usa.gov/1IZgFLV")
  expect_equal(dim(ll)[[2]], 7)
})

test_that("Update a Bitlink.", {
  li <- bitly_update_bitlink(access_token = Sys.getenv("access_token"), bitlink = "bit.ly/DPetrov", title = stri_rand_strings(1, 8, pattern = "[A-Za-z0-9]"))
  expect_gte(dim(li)[[2]], 7)
})

test_that("Given a bitly URL or hash (or multiple), returns the target (long) URL.", {
  le <- bitly_expand_link(access_token = Sys.getenv("access_token"), bitlink_id = "1.usa.gov/1IZgFLV")
  le2 <- bitly_expand_link(access_token = Sys.getenv("access_token"), bitlink_id = "bit.ly/DPetrov")
  expect_equal(dim(le)[[2]], 4)
  expect_named(le2, c("created_at", "link", "id", "long_url"))
})

test_that("Given a bitly URL or hash (or multiple), returns the target (long) URL.", {
  ls <- bitly_shorten_link(access_token = Sys.getenv("access_token"), long_url = "http://slovnik.seznam.cz/")
  ls2 <- bitly_shorten_link(access_token = Sys.getenv("access_token"), long_url = "https://travis-ci.org/dmpe/rbitly/builds/68231423")
