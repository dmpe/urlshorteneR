...
#' @title Assign bit.ly API tokens 
#'   using OAuth2.0
#' 
#' @description 
...
#' Before choosing 2. step, you can try using my API keys (the default option). No 
...
#' @title Generalized function for executing 
#'   GET/POST requests
#' 
...
doRequest <- function(verb, url, queryParameters = NULL, patch_body = NULL, showURL = NULL) {
  ...
  switch(verb,
         "PATCH" = {
           return_request <- httr::PATCH(url, query = queryParameters, body = patch_body, encode = "json", content_type_json(), 
