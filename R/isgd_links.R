...
#' 
#' @examples
#' ## asd <- links_ShortenIsgd(longUrl = "http://debil.cz/",showRequestURL = TRUE)
#' 
#' @export
links_ShortenIsgd <- function(longUrl = "", logstats = "0", shorturl = NULL, showRequestURL = FALSE) {
  links_shorten_url <- "http://is.gd/create.php?format=json"
  
  query <- list(url = longUrl, logstats = logstats, shorturl = shorturl)
  
  df_link_shorten <- doRequest("GET", links_shorten_url, service = NULL, queryParameters = query, showURL = showRequestURL)
